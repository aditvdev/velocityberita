...
$my_orders_columns = apply_filters(
	...
	array(
		'order-number'  => esc_html__( 'Order', 'justg' ),
		'order-date'    => esc_html__( 'Date', 'justg' ),
		'order-status'  => esc_html__( 'Status', 'justg' ),
		'order-total'   => esc_html__( 'Total', 'justg' ),
		'order-actions' => '&nbsp;',
	)
);
						...
						<td class="<?php echo esc_attr( $column_id ); ?>" data-title="<?php echo esc_attr( $column_name ); ?>">
							...
							<?php elseif ( 'order-number' === $column_id ) : ?>
								<a href="<?php echo esc_url( $order->get_view_order_url() ); ?>">
									<?php echo _x( '#', 'hash before order number', 'justg' ) . $order->get_order_number(); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
								</a>
							...
							<?php elseif ( 'order-total' === $column_id ) : ?>
								...
								printf( _n( '%1$s for %2$s item', '%1$s for %2$s items', $item_count, 'justg' ), $order->get_formatted_order_total(), $item_count ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
								?>
